...
module Repositories
  class CreateService
    ...
    def self.call(user:, github_id:, github_client:)
      new(user:, github_id:, github_client:).call
    end

    def initialize(user:, github_id:, github_client:)
      @user = user
      @github_id = github_id
    ...
    def call
      ...
      create_repository(github_repo)
    end

    private

    attr_reader :user, :github_id, :github_client
    ...
    def unsupported_language(github_repo)
